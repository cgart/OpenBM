...
void IBus::onReceive(const boost::system::error_code& error, std::size_t bytes_transferred)
{
    ...
    {
        std::cerr << "There was an error while receiving bytes: " << error << std::endl;
        stop();
        return;
    }
    m_Timer.cancel();
    ...
    while (m_running)
    {
        ...
        if (m_State == IDLE && m_TxQueue.size())
        {
            ...
            }catch(boost::asio::error::misc_errors& ec)
            {
                ...
                {
                    std::cerr << "Connection was closed by the peer!" << std::endl;
                    stop();
                }
            }
...
//------------------------------------------------------------------------------
bool IBus::open(const std::string& port)
{
    try
    {
        LOG_DEBUG("Open serial port");
        m_Serial.open(port);
    ...
    {
        std::cerr << "Error when opening port: " << ec << std::endl;
        return false;
    }
