...
extern void cordicck(_aAccum* x, _aAccum* y, _aAccum* z, uint8_t iterations, uint8_t mode);
extern void cordichk(_aAccum* x, _aAccum* y, _aAccum* z, uint8_t iterations, uint8_t mode);
...
#endif
#ifdef MULKD
_aAccum mulkD(_aAccum x, _aAccum y)
{
#if BYTE_ORDER == BIG_ENDIAN
#  define LO 0
#  define HI 1
...
#  define HI 0
  ...
  //*((_aAccum*)xs) = absk(x);
  //*((_aAccum*)ys) = y;
  x = sl(xs.word[HI])*y + sl(xs.word[LO])*ys.word[HI];
  //*((_aAccum*)xs) = ul(xs[LO])*ul(ys[LO]);
  xs.data = ul(xs.word[LO])*ul(ys.word[LO]);
  if(positive)
     return x + us(xs.word[HI]);
  else
     return -(x + us(xs.word[HI]));
#undef HI
#undef LO
...
#endif
#ifdef MULKS
_aAccum mulkS(_aAccum x, _aAccum y)
{
#if BYTE_ORDER == BIG_ENDIAN
#  define LO 0
#  define HI 1
...
_aAccum divkD(_aAccum x, _aAccum y) {
  _aAccum result;
  int i,j=0;
...
#endif
#ifdef DIVKS
_aAccum divkS(_aAccum x, _aAccum y) {
  _aAccum result;
  int i,j=0;
...
#endif
#ifdef SINCOSK
_aAccum sincosk(_aAccum angle, _aAccum* cosp)
{
  _aAccum x;
  _aAccum y = 0;
...
#endif
#ifdef LSINCOSK
_lAccum lsincosk(_aAccum angle, _lAccum* cosp)
{
  uint8_t correctionCount = 0;
  /* move large values into [0,2 PI] */
#define MAX_CORRECTION_COUNT 1
 ...
 * microcontroller register size and maximum possible value
 */
_aAccum roundkD(_aAccum f, uint8_t n)
{
   n = ACCUM_FBIT - n;
   if(f >= 0) {
 ...
 * microcontroller register size and maximum possible value
 */
_aAccum roundkS(_aAccum f, uint8_t n)
{
   if(n > ACCUM_FBIT) {
      return 0;
   }
 ...
 * microcontroller register size and maximum possible value
 */
_aAccum roundlkS(_lAccum f, uint8_t n)
{
   if(n > LACCUM_FBIT) {
      return 0;
   }
...
#endif
#ifdef ATAN2K
_aAccum atan2kInternal(_aAccum x, _aAccum y)
{
  _aAccum z = 0;
  uint8_t i = 0;
...
_aAccum atan2k(_aAccum x, _aAccum y)
{
  uint8_t signX, signY;
  if(y == 0)
     return 0;
 ...
 * done here.
 *
 * iterations is the fractal bit count (16 for _aAccum, 24 for _lAccum)
 * and now the only variable, the execution time depends on.
 */
void cordicck(_aAccum* px, _aAccum* py, _aAccum* pz, uint8_t iterations, uint8_t mode)
{
 ...
 * iterations is the fractal bit count (16 for _aAccum, 24 for _lAccum)
 */
void cordichk(_aAccum* px, _aAccum* py, _aAccum* pz, uint8_t iterations, uint8_t mode)
{
...
#endif
#ifdef SQRT
_aAccum sqrtk_uncorrected(_aAccum a, int8_t pow2, uint8_t cordic_steps)
{
  _aAccum x, y, z;
  if(a <= 0)
    return 0;
