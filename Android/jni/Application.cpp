...
//------------------------------------------------------------------------------
Application::Application(const std::string& address, unsigned short port, unsigned numClients, unsigned alive) :
...
//------------------------------------------------------------------------------
bool Application::connectIBus(const std::string& ibusPort, IBus::CTSType cts)
{
    m_IBus.reset(new IBus());
    m_IBus->setMessageReceiveCallback(boost::bind(&Application::onReceiveIBusMessage, this, _1));
    m_IBus->setStopIBusThreadCallback(boost::bind(&Application::onStopIBusThread, this));
    if (!m_IBus->open(ibusPort, cts))
        return false;
    return true;
}

//------------------------------------------------------------------------------
void Application::disconnectIBus()
{
    m_IBus->stop();
    m_IBus->close();
    m_IBus.reset();
...
void Application::runLoop()
{
    ...
    boost::thread* ioServiceThread = NULL;
    if (m_useTCPClients)
        ioServiceThread = new boost::thread(boost::bind(&Application::runIOservice, this));
    else
        LOG_DEBUG("TCP/IP Clients are deactivated...");
...
//------------------------------------------------------------------------------
void Application::onStopIBusThread(void)
{
    if (m_stopIBusThreadCallback) m_stopIBusThreadCallback();
}

//------------------------------------------------------------------------------
void Application::sendIBusMessage(IBus::Message* msg, bool echo)
{
    //boost::mutex::scoped_lock lock(_Mutex);
...
void Application::onReceiveIBusMessage(const IBus::Message& imsg)
{
    ...
    // if callback exists, then call driectly back
    if (m_ReceiveCallback)
        m_ReceiveCallback(imsg);
